...
glm_pr_curve <- pr.curve(scores.class0 = glm_probs$Above, scores.class1 = glm_probs$Below, curve=T)
pr_auc <- as.numeric(glm_pr_curve$auc.integral)
...
```{r}
anova(glm_model$finalModel, glm_reduced_model$finalModel, test="Chisq")
```

The p-value indicates the statistical significance of the deviance difference. In this case, the p-value is less than the conventional significance level of 0.05, suggesting that there is a significant difference between the two models. Based on this analysis, we can conclude that Model 1, which includes additional predictors compared to Model 2, provides a significantly better fit to the data. The specific predictors included in Model 1 are: StatusDeveloping, infant.deaths, Alcohol, Hepatitis.B, under.five.deaths, GDP, Population, thinness.10.19.years.
...
rose_glm_pr_curve <- pr.curve(scores.class0 = pred_probs$Above, scores.class1 = pred_probs$Below, curve=T)
pr_auc <- as.numeric(rose_glm_pr_curve$auc.integral)
...
ovun_glm_pr_curve <- pr.curve(scores.class0 = pred_probs$Above, scores.class1 = pred_probs$Below, curve=T)
pr_auc <- as.numeric(ovun_glm_pr_curve$auc.integral)
...
```{r}
plot(glm_pr_curve, max.plot = TRUE, min.plot = TRUE, rand.plot = TRUE, fill.area = TRUE, color = 2, auc.main = FALSE, ylim = c(0.5, 1))
plot(rose_glm_pr_curve, add = TRUE, color = 3)
plot(ovun_glm_pr_curve, add = TRUE, color = 4)

legend_text <- c(
  paste0("LR Unbalanced (AUC = ", round(glm_pr_curve$auc.integral, 3), ")"),
  paste0("LR ROSE (AUC = ", round(rose_glm_pr_curve$auc.integral, 3), ")"),
  paste0("LR Ovun (AUC = ", round(ovun_glm_pr_curve$auc.integral, 3), ")")
)

legend("topright", legend = legend_text, col = c(2, 3, 4), lty = 1, lwd = 2, bty = "n", cex = 0.8)
```
...
Let's visualize the confusion matrix of the logistic regression model trained on the unbalanced dataset:
...
```
We will now proceed with testing other classification algorithms while retaining the Logistic Regression model trained on an unbalanced dataset for comparison.

```{r}
df_metrics <- df_metrics[df_metrics$Model_name == "Logistic regression", ]
...
nb_pr_curve <- pr.curve(scores.class0 = nb_pred_probs[, "Above"], scores.class1 = nb_pred_probs[, "Below"], curve=T)
pr_auc <- as.numeric(nb_pr_curve$auc.integral)
...
We also evaluated the performance of Naive Bayes using training subsets created using the ROSE and Ovun techniques, in comparison to the model trained on the unbalanced dataset. However, no improvement was observed.
...
lda_pr_curve <- pr.curve(scores.class0 = pred_probs$Above, scores.class1 = pred_probs$Below, curve=T)
pr_auc <- as.numeric(lda_pr_curve$auc.integral)
...
We also assessed the performance of LDA using the synthetic training dataset. However, similar to previous experiments, the results did not show any improvement. Consequently, we have decided not to retain those results for further analysis.
...
qda_pr_curve <- pr.curve(scores.class0 = pred_probs$Above, scores.class1 = pred_probs$Below, curve=T)
pr_auc <- as.numeric(qda_pr_curve$auc.integral)
...
knn_pr_curve <- pr.curve(scores.class0 = knn_pred_probs[,"Above"], scores.class1 = knn_pred_probs[,"Below"], curve=T)
pr_auc <- as.numeric(knn_pr_curve$auc.integral)
...
After evaluating the results obtained from training various classifiers, it was observed that the synthetic balancing of the training dataset using two different techniques did not lead to a significant improvement in performance for the tested models. As a result, these models were not included in our final analysis.
...
```{r, echo=FALSE}
plot(glm_pr_curve, max.plot = TRUE, min.plot = TRUE, rand.plot = TRUE, fill.area = TRUE, color = 2, auc.main = FALSE, ylim = c(0.0, 1))
plot(nb_pr_curve, add = TRUE, color = 3)
plot(lda_pr_curve, add = TRUE, color = 4)
plot(qda_pr_curve, add = TRUE, color = 5)
plot(knn_pr_curve, add = TRUE, color = 6)

legend_text <- c(
  paste0("LR (AUC = ", round(glm_pr_curve$auc.integral, 3), ")"),
  paste0("NB (AUC = ", round(nb_pr_curve$auc.integral, 3), ")"),
  paste0("LDA (AUC = ", round(lda_pr_curve$auc.integral, 3), ")"),
  paste0("QDA (AUC = ", round(qda_pr_curve$auc.integral, 3), ")"),
  paste0("KNN (AUC = ", round(knn_pr_curve$auc.integral, 3), ")")
)
