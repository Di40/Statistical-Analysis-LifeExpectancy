...
<b>However,before jumping to conclusions we need to also take in consideration other factors.</b>
...
Because there are a lot of variables included in the network plot visualization it is not intuitive for making some conclusions, therefore we decided to also make a star graph with the target variable Life.expectancy that will also be useful later for Linear Regression.
...
numeric_vars_1 <- numeric_vars[, c("Life.expectancy", "infant.deaths", "Alcohol",
...
cor_matrix_1 <- cor(numeric_vars_1, use = "pairwise.complete.obs")

# Extract the correlation values for "Life.Expectancy"
...
V(graph)$name <- colnames(numeric_vars_1)
V(graph)$size <- c(5, 10 + 40 * abs(cor_values))

# Set the edge attributes (thickness based on correlation strength)
...
plot_size <- c(15, 15)  # Adjust the dimensions as needed

# Plot the star graph with increased size
...
```{r}
kable(df_metrics_regression[order(-df_metrics_regression$Adj_R2), ])
```
...
mean_test <- mean(test$Life.expectancy)
std_test <- sd(test$Life.expectancy)

train_scaled <- train
test_scaled <- test

train_scaled[, numeric_cols] <- scale(train[, numeric_cols])
test_scaled[, numeric_cols] <- scale(test[, numeric_cols])
```
...
MSE with the best lambda: 13.29373
...
lasso_mse
```
The value of best lambda is equal to: 0.02213345
MSE with the best lambda: 13.44903
...
```{r}
kable(df_metrics_regression[order(df_metrics_regression$MSE)])
```
...
summary(glm_model)
```
Some predictor variables, such as Adult.Mortality", "percentage.expenditure" and "HIV.AIDS", have statistically significant coefficients (p < 0.05), suggesting a significant association with the likelihood of life expectancy being above or below the threshold.
...
glm_probs <- predict(glm_model, newdata = df_test, type = "prob")
pr_auc <- as.numeric(pr.curve(scores.class0 = glm_probs$Above, scores.class1 = glm_probs$Below, curve=T)$auc.integral)
...
We can see that AIC increased from 929.45 to 930.9. Usually, when comparing two models, a difference of more than 2 in their AIC value is enough to say the model with the lower AIC is better. However, in our case this rule of thumb was not met. Therefore, we have decided to proceed with the full model. As a future work,  more sophisticated feature selection methods shall be explored. However, that was not one of the goals of this project.
...
pred_probs <- predict(glm_reduced_model, newdata = df_test_subset_glm, type = "prob")
pr_auc <- as.numeric(pr.curve(scores.class0 = pred_probs$Above, scores.class1 = pred_probs$Below, curve=T)$auc.integral)
...
pred_probs <- predict(glm_rose_model, newdata = df_test, type = "prob")
pr_auc <- as.numeric(pr.curve(scores.class0 = pred_probs$Above, scores.class1 = pred_probs$Below, curve=T)$auc.integral)
...
pred_probs <- predict(glm_rose_reduced_model, newdata = df_test_subset_rose, type = "prob")
pr_auc <- as.numeric(pr.curve(scores.class0 = pred_probs$Above, scores.class1 = pred_probs$Below, curve=T)$auc.integral)
...
pred_probs <- predict(glm_ovun_model, newdata = df_test, type = "prob")
pr_auc <- as.numeric(pr.curve(scores.class0 = pred_probs$Above, scores.class1 = pred_probs$Below, curve=T)$auc.integral)
...
pred_probs <- predict(ovun_reduced_model, newdata = df_test_subset_ovun, type = "prob")
pr_auc <- as.numeric(pr.curve(scores.class0 = pred_probs$Above, scores.class1 = pred_probs$Below, curve=T)$auc.integral)
...
After evaluating the models, we found that they have comparable F1 scores. However, when considering the PR AUC value, the unbalanced model stands out and performs better than the other models.

Comparing the Ovun.sample and ROSE approaches, we see that they exhibit similar performance. However, based on the lower AIC value and higher McFadden's R2, we would prefer the Ovun.sample approach over the ROSE approach.

Additionally, when examining the impact of the selected features, we noticed a slight improvement in the F1 score of the standard Logistic regression model. However, it resulted in a decrease in the PR AUC value. Interestingly, when using the subset of selected features, the performance of the balanced models (ROSE and Ovun) dropped.

Based on these findings, we will proceed with the classification analysis, focusing solely on the logistic regression model with the selected subset of features and excluding the other models from further consideration.

Let's visualize its confusion matrix:
...
```{r}
# Filter the dataframe to keep only the row with Model_name "Logistic regression"
df_metrics <- df_metrics[df_metrics$Model_name == "Logistic regression - Reduced", ]
...
fourfoldplot(table(df_test$Pension_Status, nb_pred), color = c("#CC6666", "#99CC99"),
...
pr_auc <- as.numeric(pr.curve(scores.class0 = nb_pred_probs[, "Above"], scores.class1 = nb_pred_probs[, "Below"], curve=T)$auc.integral)
...
fourfoldplot(table(df_test_subset_glm$Pension_Status, nb_reduced_pred), color = c("#CC6666", "#99CC99"),
             conf.level = 0, margin = 1, main = "Confusion Matrix")
```
...
nb_pred_probs <- predict(nb_model_reduced, newdata = df_test_subset_glm, type = "raw")
pr_auc <- as.numeric(pr.curve(scores.class0 = nb_pred_probs[, "Above"], scores.class1 = nb_pred_probs[, "Below"], curve=T)$auc.integral)
...
pred_probs <- predict(lda_model, newdata = df_test, type = "prob")
pr_auc <- as.numeric(pr.curve(scores.class0 = pred_probs$Above, scores.class1 = pred_probs$Below, curve=T)$auc.integral)
...
pred_probs <- predict(lda_reduced_model, newdata = df_test_subset_glm, type = "prob")
pr_auc <- as.numeric(pr.curve(scores.class0 = pred_probs$Above, scores.class1 = pred_probs$Below, curve=T)$auc.integral)
...
pred_probs <- predict(qda_model, newdata = df_test, type = "prob")
pr_auc <- as.numeric(pr.curve(scores.class0 = pred_probs$Above, scores.class1 = pred_probs$Below, curve=T)$auc.integral)
...
ggplot(df_train, aes(x = Status, fill = Pension_Status)) +  geom_bar(alpha = 0.7)
```
...
pred_probs <- predict(qda_reduced_model, newdata = df_test_subset_glm, type = "prob")
pr_auc <- as.numeric(pr.curve(scores.class0 = pred_probs$Above, scores.class1 = pred_probs$Below, curve=T)$auc.integral)
...
knn_pred_probs <- predict(final_model, newdata = df_test[, -ncol(df_test)], type="prob")
pr_auc <- as.numeric(pr.curve(scores.class0 = knn_pred_probs[,"Above"], scores.class1 = knn_pred_probs[,"Below"], curve=T)$auc.integral)
...
```{r}
numeric_cols <- names(df_test_subset_glm)[sapply(df_test_subset_glm, is.numeric)]

df_train_subset_glm[, numeric_cols] <- scale(df_train_subset_glm[, numeric_cols])
df_test_subset_glm[, numeric_cols] <- scale(df_test_subset_glm[, numeric_cols])
```
...
final_model <- knn_reduced_model$finalModel
final_knn_pred <- predict(final_model, newdata = df_test_subset_glm[, -1], type="class")
cm <- confusionMatrix(data=final_knn_pred, reference = df_test_subset_glm$Pension_Status)
fourfoldplot(cm$table, color = c("#CC6666", "#99CC99"),
             conf.level = 0, margin = 1, main = "Confusion Matrix")
```
The model's performance has slightly improved. In general, a lower number of features can be beneficial for k-NN classification. With a reduced feature set, the curse of dimensionality is mitigated, which can lead to improved performance of the k-NN algorithm. 
...
knn_pred_probs <- predict(final_model, newdata = df_test_subset_glm[, -1], type="prob")
pr_auc <- as.numeric(pr.curve(scores.class0 = knn_pred_probs[,"Above"], scores.class1 = knn_pred_probs[,"Below"], curve=T)$auc.integral)
...
After evaluating the results and comparing the models trained on all features versus the reduced subset of features, it was found that the feature selection process did not significantly improve the performance for most models, except for Logistic Regression. Therefore, for our final analysis, we will exclude the models trained on the reduced feature subset.
...
rounded_df <- sorted_df
rounded_df[, -1] <- round(rounded_df[, -1], 3)
rounded_df <- rounded_df[!rounded_df$Model_name %in% c("Naive Bayes - Reduced", "QDA - Reduced", "K-Nearest Neighbors - Reduced", "LDA - Reduced"), ]
kable(rounded_df)
```

In our final analysis, we have chosen to prioritize the PR_AUC and F1 score as most significant metrics, especially for evaluating models with unbalanced datasets. Taking into account the values of these two metrics, we have identified the top-performing models as follows:

1. Logistic Regression - Reduced
2. Linear Discriminant Analysis
3. K-Nearest Neighbors

To determine the overall winner, it is important to carefully assess the strengths of each model in comparison to the others. The Logistic Regression - Reduced model displays superior performance in terms of both F1 score and PR AUC. This model exhibits good precision, recall, and accuracy, while effectively balancing false positives and false negatives. Additionally, its reduced feature set helps address the curse of dimensionality and improves computational efficiency.
